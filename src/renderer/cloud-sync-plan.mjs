...
export function planCloudChanges(collectionNames, data, baseline, toCloud) {
  ...
  collectionNames.forEach((collectionName) => {
    (data[collectionName] || []).forEach((source) => {
      ...
      if (baseline.get(key) !== serialized) operations.push({ type: 'set', collectionName, recordId: record.id, data: record });
    });
  });
  baseline.forEach((_, key) => {
    if (nextBaseline.has(key)) return;
    const separator = key.indexOf('/');
    operations.push({ type: 'delete', collectionName: key.slice(0, separator), recordId: key.slice(separator + 1) });
  });
