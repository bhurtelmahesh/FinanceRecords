...
import { cloudMetaOperation, cloudRecordKey, planCloudChanges } from '../src/renderer/cloud-sync-plan.mjs';

const collections = ['salary', 'expenses'];
const clean = (_collection, record) => ({ id: record.id, value: record.value });

test('cloud plan writes changed records, skips unchanged records, and adds new records', () => {
...
test('cloud plan deletes records removed on this Mac', () => {
  const baseline = new Map([[cloudRecordKey('expenses', 'gone'), JSON.stringify({ id: 'gone', value: 1 })]]);
  const { operations, nextBaseline } = planCloudChanges(collections, {}, baseline, clean);
  assert.deepEqual(operations, [{ type: 'delete', collectionName: 'expenses', recordId: 'gone' }]);
  assert.equal(nextBaseline.size, 0);
});

test('cloud metadata uses the app/meta operation shape', () => {
  assert.deepEqual(cloudMetaOperation({ meta: { version: 3, startedAt: 'start' } }, 'now'), {
    type: 'set', collectionName: 'app', recordId: 'meta',
    data: { version: 3, startedAt: 'start', importedAt: '', sourceFile: '', updatedAt: 'now' }
  });
});
